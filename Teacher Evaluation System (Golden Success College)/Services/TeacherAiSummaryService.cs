...
namespace Teacher_Evaluation_System__Golden_Success_College_.Services
{
    public interface ITeacherAiSummaryService
    {
        ...
        byte[] GeneratePdfReport(string teacherName, List<Evaluation> evaluations);
    }
    ...
    public class TeacherAiSummaryService : ITeacherAiSummaryService
    {
        ...
        public byte[] GeneratePdfReport(string teacherName, List<Evaluation> evaluations)
        {
            var summary = GenerateSummary(teacherName, evaluations);
            var advice = GenerateAdvice(evaluations);
            var avgScore = evaluations.Any() ? evaluations.Average(e => e.AverageScore) : 0;
            var totalEvals = evaluations.Count;
            ...
            var document = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header().Row(row =>
                    {
                        ...
                        {
                            col.Item().Text("GOLDEN SUCCESS COLLEGE").FontSize(20).SemiBold().FontColor(Colors.Blue.Medium);
                            col.Item().Text("Teacher Performance Evaluation Report").FontSize(14).SemiBold().FontColor(Colors.Grey.Medium);
                        });
                    ...
                    page.Content().PaddingVertical(20).Column(col =>
                    {
                        ...
                        {
                            row.RelativeItem().Text(teacherName).FontSize(18).SemiBold();
                            row.RelativeItem().AlignRight().Text($"Overall Rating: {avgScore:F2} / 4.00").FontSize(14).SemiBold().FontColor(Colors.Green.Medium);
                        });

                        col.Item().PaddingTop(15).Text("Performance Summary").FontSize(14).SemiBold().FontColor(Colors.Blue.Medium);
                        col.Item().PaddingTop(5).Text(summary.Replace("**", "").Replace("📊", "").Replace("🏆", "").Replace("🛠️", "").Replace("📅", ""));

                        col.Item().PaddingTop(20).Text("Professional Growth Roadmap").FontSize(14).SemiBold().FontColor(Colors.Green.Medium);
                        col.Item().PaddingTop(5).Text(advice.Replace("**", "").Replace("🚀", "").Replace("⭐", ""));

                        col.Item().PaddingTop(30).BorderTop(1).BorderColor(Colors.Grey.Lighten2).Text(x =>
                        {
                            x.Span("Generated by AI Insight Engine").Style(TextStyle.Default.Italic().FontSize(9).FontColor(Colors.Grey.Medium));
                        });
                    });
